...
comparisonName = {'GLP_greater', 'Saline_greater'};
...
for contrastNum=1:length(contrastList)
    ...
    for roiNum=1:length(roiList)
        ...
        for comparisonNum=1:length(comparisonName)
            [hReg, SPM, xSPM] = results_paired(contrast{1}, comparisonNum, ROILoc{1});
            TabDat = spm_list('List',xSPM,hReg);
            if ~isempty(xSPM.XYZ)
                target_xSPM_loc = sprintf('/home/kangik/2016_CHJ/GLP_1/IMG_DATA/paired_%s_%s_%d_xSPM.mat', contrast{1}, ROI{1}, comparisonNum)
